...
export const scrollbarWCDemo: DemoRoute = {
  ...
  render: () => `
    <div id="scrollbar-wc-demo-content"></div>
  `,
  ...
  setup: () => {
    ...
    const container = document.getElementById('scrollbar-wc-demo-content')!
    ...
    const vScroll = sb({ kind: 'vertical', max: 200, value: 50, showTooltip: true, hover: true })
    Object.assign(vScroll.style, { height: '100%' })

    hWrapper.appendChild(contentArea)
    hWrapper.appendChild(vScroll)

    const bottomRow = document.createElement('div')
    Object.assign(bottomRow.style, { display: 'flex' })

    const hScroll = sb({ kind: 'horizontal', max: 300, value: 100, showTooltip: true, hover: true })
    Object.assign(hScroll.style, { flex: '1' })

    const corner = document.createElement('div')
    Object.assign(corner.style, { width: '14px', height: '14px', flexShrink: '0' })

    bottomRow.appendChild(hScroll)
    ...
    eventInfo.textContent = 'Listening via addEventListener("sb-change") on the <scrollbar-wc> element:'
    eventSection.appendChild(eventInfo)

    const eventRow = document.createElement('div')
    eventRow.className = 'btn-demo-row'

    const eventSb = sb({ kind: 'horizontal', max: 100, value: 50, showTooltip: true }, '300px')
    const eventLabel = document.createElement('span')
    eventLabel.style.cssText = 'font-size:12px;color:var(--button-fg-color);min-width:100px;'
    eventLabel.textContent = 'value: 50'

    eventSb.addEventListener('sb-change', ((e: CustomEvent) => {
      eventLabel.textContent = `value: ${Math.round(e.detail.value)}`
    }) as EventListener)

    eventSb.addEventListener('sb-dragstart', (() => {
      eventLabel.style.fontWeight = 'bold'
    }) as EventListener)

    eventSb.addEventListener('sb-dragend', (() => {
      eventLabel.style.fontWeight = ''
    }) as EventListener)
    ...
    attrInfo.style.cssText = 'font-size:11px;color:var(--button-disabled-fg-color);margin-top:16px;margin-bottom:8px;'
    attrInfo.textContent = 'Created via HTML attributes: <scrollbar-wc kind="horizontal" size="medium" max="200" value="80" show-tooltip>'
    eventSection.appendChild(attrInfo)
    ...
    attrSb.setAttribute('value', '80')
    attrSb.setAttribute('show-tooltip', '')
    attrSb.style.width = '300px'

    const attrLabel = document.createElement('span')
    attrLabel.style.cssText = 'font-size:12px;color:var(--button-fg-color);min-width:100px;'
    attrLabel.textContent = 'value: 80'

    attrSb.addEventListener('sb-change', ((e: CustomEvent) => {
      attrLabel.textContent = `value: ${Math.round(e.detail.value)}`
    }) as EventListener)

    attrRow.appendChild(attrSb)
    attrRow.appendChild(attrLabel)
    eventSection.appendChild(attrRow)
    allSbs.push(attrSb)
